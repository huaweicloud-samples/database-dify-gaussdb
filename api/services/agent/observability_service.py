...
WHERE
    ...
    def _workflow_execution_metadata_numeric_sql(path: tuple[str, ...], numeric_type: str) -> str:
        ...
        if dify_config.DB_TYPE == "gaussdb":
            # GaussDB O模式不支持 #>> 操作符，用 -> 链替代（G2 实测 -> / ->> 可用）
            # path=(a,b,c) -> CAST(col AS JSONB) -> 'a' -> 'b' ->> 'c'
            accessors = "".join(f" -> '{p}'" for p in path[:-1])
            accessors += f" ->> '{path[-1]}'"
            value = f"CAST(wne.execution_metadata AS JSONB){accessors}"
            return f"CAST(NULLIF({value}, '') AS {numeric_type})"
