...
def upgrade():
    ...
    if _is_pg(conn):
        ...
        with op.batch_alter_table('workflows', schema=None) as batch_op:
            batch_op.add_column(sa.Column('marked_name', sa.String(length=255), nullable=False))
            batch_op.add_column(sa.Column('marked_comment', sa.String(length=255), nullable=False))
